...
class TestT5DumpValue:
	...
	@pytest.mark.parametrize("component,dec", [("RegionA", 45.0), ("RegionB", 44.0)])
	def test_dump_value_object_component_loses_shared_frame(self, sky, component, dec):
		'''
		...
		generic Cartesian frame, NOT the original sky frame. Frame semantics
		(isaskyframe, overlap) silently degrade, and even circlepars() geometry
		distorts (radius inflated by exactly 1/cos(dec)); only raw stored axis
		values survive. If this test ever fails because the frame IS preserved,
		AST changed its dump behavior: update dump_value's docstring and
		SPEC-04A Appendix B accordingly.
		'''
		from cornish.channel.channel_io import ListSource
		c1 = Ast.Circle(sky, 1, np.deg2rad([30.0, 45.0]), [np.deg2rad(1.0)])
		c2 = Ast.Circle(sky, 1, np.deg2rad([32.0, 44.0]), [np.deg2rad(1.0)])
		original = {"RegionA": c1, "RegionB": c2}[component]
		compound = Ast.CmpRegion(c1, c2, Ast.OR)
		block = bridge.dump_value(compound, component)
		read_back = Ast.Channel(ListSource(block), None).read()
		...
		assert read_back.overlap(original) != 5
		...
		radius_deg = np.rad2deg(read_back.circlepars()[1])
		assert radius_deg == pytest.approx(1.0 / np.cos(np.deg2rad(dec)), rel=1e-3)
		# raw stored axis values are the one thing that survives unchanged
		assert np.array_equal(read_back.getregionpoints(), original.getregionpoints())
